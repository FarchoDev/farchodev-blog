      ...
      <Link to={`/post/${post.slug}`} data-testid={`featured-post-${post.id}`}>
        <div className="relative rounded-2xl overflow-hidden h-[500px] card-hover">
          {post.featured_image_url ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-blue-600 via-blue-500 to-yellow-500" style={{background: 'linear-gradient(135deg, #0E4CB2 0%, #1a5fc9 50%, #B89A4B 100%)'}} />
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent" />
          <div className="absolute bottom-0 left-0 right-0 p-8 text-white">
            <span className="inline-block px-3 py-2 rounded-full text-sm font-semibold mb-4" style={{background: '#B89A4B'}}>{post.category}</span>
